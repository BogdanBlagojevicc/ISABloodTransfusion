...
const CenterComplaintList = () => {
    ...
    const rowEvent = {
    onClick: (e, row) => {
        navigate(`/CenterComplaint/`+row.id);
    },
    }; 

    useEffect(()=>{
        var test = JSON.parse(localStorage.getItem('testToken'))
        fetch("http://localhost:8081/api/centers/centersRegularUser/" + localStorage.getItem('reg_user_username'),{
          headers : { 
...
export default CenterComplaintList